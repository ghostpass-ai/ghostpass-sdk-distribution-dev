...
```kotlin
dependencies {
    implementation("com.ghostpass:gopass-user-sdk-dev:<version>")
...
dependencyResolutionManagement {
    ...
    repositories {
        ...
        mavenCentral()
        maven {
            url = uri("https://ghostpass-ai.github.io/ghostpass-sdk-distribution-dev/user-sdk")
        }
    }
}
```
...
try {
    ...
    Log.e(TAG, "code=${e.code}, message=${e.message}, internalCode=${e.internalCode}")
    // → "GP101, 초기화를 실패했습니다., IN-101"  ← 문의 시 에러 코드 전달
}
```
...
try {
    ...
    Log.e(TAG, "code=${e.code}, message=${e.message}, internalCode=${e.internalCode}")
    // → "code=GP401, message=핸즈프리 인증 처리에 실패했습니다., internalCode=IN-801"
}
```
...
A. `GoPassException`의 `code`와 `message`, `internalCode`를 함께 전달해 주세요.
